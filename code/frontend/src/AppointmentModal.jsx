...
import { appointmentsApi, formatApptDate, formatApptTime } from "./lib/appointmentsApi";
...
export default function AppointmentModal({
      ...
      .then((data) => {
        ...
        if (providerName) {
          ...
          const providerDates = new Set(
            data
              .filter((s) => s.provider_name === providerName)
              .map((s) => s.available_date)
          );
          setActiveDate(providerDates.has(today) ? today : [...providerDates].sort()[0] ?? today);
        }
      })
      .catch(() => setSlots([]))
  ...
  const filtered = providerName
    ? slots.filter((s) => s.provider_name === providerName)
    : specialty === "All"
    ? slots
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (rescheduleId) {
        await appointmentsApi.cancelAppointment(rescheduleId);
      }
      await appointmentsApi.createAppointment({
        ...
        appointment_time: selectedSlot.available_time,
        notes: notes || null,
      });
      onBooked?.();
      onClose();
              ...
              {currentWeekKeys.map((dateStr) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => { setActiveDate(dateStr); setSelectedSlot(null); }}
                  >
                    <span className="am-dc-mon">{MON_ABBR[d.getMonth()]}</span>
                    <span className={`am-dc-num${isToday && !isActive ? " today" : ""}`}>
                      {d.getDate()}
                    </span>
          ...
          <div className="am-slots">
            {loading ? (
              ...
              availableDateSet.size === 0 ? (
                <p className="am-empty-slots">No available slots for this provider.</p>
              ) : timeSlotsForDate.length === 0 ? (
                <p className="am-empty-slots">No available times on this date.</p>
              ) : (
                <div className="am-time-list">
                  {timeSlotsForDate.map((slot) => (
                    ...
                    >
                      ...
                      {slot.location && (
                        <span className="am-time-btn-loc">
                          <MapPin size={11} />
                          {slot.location}
                        </span>
                      )}
              ...
              <p className="am-empty-slots">No slots available for this specialty.</p>
            ) : (
                  ...
                  <div className="am-slot-grid">
                    {dateSlots.map((slot) => (
                      ...
                      >
                        ...
                        <span className="am-slot-provider">{slot.provider_name}</span>
                        {slot.location && (
                          <span className="am-slot-loc">
                            <MapPin size={10} />
                            {slot.location}
                          </span>
                        )}
              ...
              <>
                <div className="am-selected-card">
                  ...
                  {selectedSlot.specialty && (
                    <span className="am-badge">{selectedSlot.specialty}</span>
                  )}
