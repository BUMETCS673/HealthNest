# Our Team

## Aaron Brown

My name is Aaron. I'm originally from Minneapolis, Minnesota but lived in Boston for 10 years before moving to NYC. I studied music for my undergrad and worked as a music producer for a number of years. This work led me to developing an interest in software development in order to create my own website and digital audio plugins. I self taught for a couple of years with online courses and eventually started my masters in Software Development at BU MET last spring.

## Stephanie Wang

I’m Stephanie. I grew up in Taiwan and have lived in California for the past 10 years. I currently live in the San Francisco Bay Area. I studied communication for my undergraduate degree and later worked as a banker, so I never imagined I would end up becoming interested in tech. Last year, I took an online user experience course, which made me start thinking it might be fun to learn some programming languages and create something from just an idea. That experience eventually led me to pursue my master’s in Software Development this year.

## Deven Shah
My name is Deven Shah. I'm currently pursuing my MS in Computer Science with a concentration in AI/ML at BU. I recently started working as a Technical Analyst at Accenture. Concurrently, I am involved in computer vision research with two labs at BU, focusing on medical imaging and digital forensics. Previously, I co-founded Suno Analytics as CTO, and have gained software experience at companies like Patelco and NetApp.

## Chamarr Auber

My name is Chamarr. Im originally from Maryland, but I currently live in Houston, Texas for my current job. I graduated from the university of maryland with a computer science degree. I currently work as a software engineer. I was originally a pre-med major, but I made the switch to computer science because i found a love for coding and solving problems.
